...
impl Snapshots {
    ...
    pub(crate) fn from_newick_iter_opts<'a>(
        ...
        let mut interner = Interner::with_capacity(entries.len(), store_lengths);
...
impl Interner {
    fn with_capacity(n_trees: usize, store_lengths: bool) -> Self {
        Self {
            hasher: FxBuildHasher,
            table: HashTable::new(),
            bipartitions: Vec::new(),
    ...
    fn push(&mut self, snap: Snapshot) {
        ...
        let lengths = if self.store_lengths {
            lengths
        } else {
            Vec::new()
        };
