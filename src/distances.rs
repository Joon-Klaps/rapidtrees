...
///
/// `kernel.fill_row(i, row)` writes `row[j]` for every `j > i`. `row` arrives
/// holding the default cell throughout, so a kernel may accumulate into it
/// first. The diagonal stays at the default cell and the lower triangle is
/// mirrored from the upper. `progress` is bumped by each row's pair count as
/// that row finishes.
...
fn distance_rf_at(snaps: &Snapshots, progress: Option<&AtomicUsize>, level: Level) -> Vec<u32> {
    ...
    distance_rf_split(snaps, progress, min_dense, level)
}
...
/// (but at least two) gets a posting list. The row loop is compiled for
/// `level`.
...
/// One RF row over bit columns and posting lists: see [`distance_rf`].
struct RfRows<'a> {
    bits: &'a BitRows,
    postings: &'a Postings,
...
impl RowKernel for RfRows<'_> {
    ...
    fn fill_row(&self, i: usize, row: &mut [u32]) {
        ...
        for &col in self.postings.of_tree(i).0 {
            for &j in self.postings.after(col, i).0 {
                row[j as usize] += 1;
            }
        }

        let self_count = self.self_count;
        for (j, slot) in row.iter_mut().enumerate().skip(i + 1) {
            let shared = self.bits.shared(i, j) + *slot;
...
fn weighted_distances(
    ...
    weighted_distances_split(snaps, progress, min_dense, level, overlap, finish)
}
...
/// clamp stops rounding from handing `finish` a negative. The row loop is
/// compiled for `level`.
...
/// One WRF or KF row over dense columns and posting lists: see
    ...
    F: Fn(f64) -> f64 + Sync,
{
    ...
    fn fill_row(&self, i: usize, row: &mut [f64]) {
        let (overlap, self_total) = (&self.overlap, self.self_total);

        // Shared terms from the posting lists first, accumulated in place.
        let (cols, lengths) = self.postings.of_tree(i);
        for (&col, &length) in cols.iter().zip(lengths) {
            let (trees, others) = self.postings.after(col, i);
            for (&j, &other) in trees.iter().zip(others) {
                row[j as usize] += overlap(length, other);
            }
        }

        let own = row_slice(self.dense, i, self.stride);
        for (j, slot) in row.iter_mut().enumerate().skip(i + 1) {
            let shared = sweep(own, row_slice(self.dense, j, self.stride), overlap) + *slot;
            *slot = (self.finish)((self_total[i] + self_total[j] - 2.0 * shared).max(0.0));
        }
    }
}
...
fn distance_wrf_at(snaps: &Snapshots, progress: Option<&AtomicUsize>, level: Level) -> Vec<f64> {
    weighted_distances(snaps, progress, level, f64::min, |d| d)
}
...
fn distance_kf_at(snaps: &Snapshots, progress: Option<&AtomicUsize>, level: Level) -> Vec<f64> {
    weighted_distances(snaps, progress, level, |a, b| a * b, f64::sqrt)
}
...
mod tests {
    use super::{
        NO_COLUMN, TREEDIST_TREES, assign_columns, distance_kf_at, distance_rf_at,
        distance_rf_split, distance_wrf_at, parse_share, weighted_distances_split,
    };
    use crate::{
        cpu::Level,
    ...
    fn weighted_at(snaps: &Snapshots, min_dense: u32) -> (Vec<f64>, Vec<f64>) {
        (
            weighted_distances_split(snaps, None, min_dense, Level::current(), f64::min, |d| d),
            weighted_distances_split(
                snaps,
                None,
                min_dense,
    ...
    fn rf_boundary_does_not_change_distances() {
        for &(n_taxa, n_trees, duplicates, seed) in &[
            ...
            let n = snaps.snapshots.len();
            for min_dense in [2, 3, n as u32 / 2, n as u32, n as u32 + 1] {
                let rf = distance_rf_split(&snaps, None, min_dense, Level::current());
