...
- **WebAssembly build + browser site.** `rapidtrees` now compiles to `wasm32-unknown-unknown` behind a `wasm` cargo feature, and ships a minimal static site (`www/`) that computes a pairwise distance matrix (RF / WRF / KF) from an uploaded BEAST `.trees` file entirely in the browser — no server, no upload. The matrix is streamed to disk as CSV (optionally gzipped) rather than rendered, so multi-gigabyte results stay off the page. Build with `wasm-pack build --target web --out-dir www/pkg --no-default-features --features wasm` (or `pixi run build-wasm`) and deploy via the new `deploy-pages.yml` GitHub Actions workflow. Because upstream `phylotree` cannot compile to wasm, the wasm build swaps in a small in-crate Newick parser (`src/phylotree-patch.rs`); a native test cross-checks that it produces byte-identical distance matrices to `phylotree`.
