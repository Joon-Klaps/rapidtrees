...
- **GPU diagnostics:** added `rapidtrees::gpu_adapter_label()` (Rust, `gpu` feature only), which forces GPU initialisation and returns a human-readable adapter label (e.g. `Tesla P100-SXM2-16GB (Vulkan, DiscreteGpu)`) or `None` when computation would fall back to the CPU. The memory/time benchmark now calls it on `--gpu` and prints `GPU CONFIRMED: …` / `GPU NOT USED …` to stdout, so the run log states plainly whether the GPU engaged instead of leaving it to be inferred from timings.
- **GPU fix (RF):** `try_pairwise_rf` now returns an all-zero matrix when no non-universal splits remain (e.g. all trees identical) instead of handing wgpu a zero-length storage buffer, which it rejected with a validation error. Universal splits cancel in RF, so the result is unchanged.
- **GPU fix (watchdog / device-lost guard):** the whole n×n matrix is computed in a single `queue.submit`. On a headless compute GPU (e.g. a Tesla P100) a dispatch that runs too long trips the driver watchdog, which loses the device — and wgpu *panics* on submit to a lost device, crashing the process (it cannot be caught and recovered). The heavy dense WRF/KF path (`O(n² × n_splits)`) hit this on large cells. Added a conservative per-dispatch work bound (`n² × inner`): above it, the metric falls back to the CPU (always correct, just slower). RF is unaffected at all realistic sizes; the long-term fix is a chunked/banded dispatch that bounds rows per submit.
- **GPU fix (buffer limits — correctness):** the GPU path requested `Limits::downlevel_defaults()`, which caps a storage-buffer binding at 128 MiB. On larger inputs the packed-row, dense-length, and n×n output buffers exceed that, so the dispatch failed validation and the readback returned a **garbage (all-zero) matrix that was silently accepted as a successful GPU result** — producing wrong distances and meaningless GPU timings. Fixed three ways: (1) request the adapter's real limits (NVIDIA Vulkan reports multi-GiB binding sizes), so large buffers run on the GPU where the hardware supports them; (2) every buffer is checked against the device's actual `max_storage_buffer_binding_size` / `max_buffer_size` before dispatch and falls back to the CPU if it won't fit; (3) the dispatch + readback run inside a wgpu validation error scope, so any residual device error returns `None` (CPU fallback) instead of a garbage matrix.
- **GPU acceleration (`use_gpu=True`):** All six `pairwise_*_from_newick_iter` Python functions now accept an optional `use_gpu: bool = False` keyword argument. When `True`, rapidtrees attempts to dispatch the pairwise computation to a wgpu-backed GPU (Vulkan / Metal / DX12 / OpenGL — whichever the runtime finds). If no suitable adapter is available, or if the tree count is below the threshold (64), it silently falls back to the CPU path with no change in results.
  ...
  - **Default unchanged:** `use_gpu=False` — all existing callers see identical results and behaviour. Whether the GPU is used is controlled solely by this argument; there are no environment-variable overrides.
  - **Visible fallback:** when `use_gpu=True` but no compatible GPU adapter is found, rapidtrees now prints a one-time warning to stderr (`GPU requested but no compatible GPU adapter was found; falling back to the CPU…`) instead of falling back silently. The warning fires once per process and hints that a missing Vulkan loader/ICD is the usual Linux cause (wgpu has no CUDA backend).
  - Requires the `gpu` Cargo feature, now a **default feature** so `cargo build`, `cargo bench`, and the `rapidtrees` CLI compile the GPU path out of the box (the published Python wheel already enabled it via `extension-module`). Build with `--no-default-features` to opt out (e.g. CPU-only benchmarking). ([#15](https://github.com/Joon-Klaps/rapidtrees/pull/15))
- **Memory (snapshot construction):** `Snapshots::from_newick_iter` now interns bipartitions incrementally, parsing trees in bounded parallel chunks and folding each chunk into the split-ID table before the next is parsed. Previously every tree's raw, un-deduplicated `Vec<Bitset>` was held in memory at once, so peak construction memory could be ~15× the deduplicated result (e.g. 4.4 GB peak for a 287 MB result at 2000 taxa × 5000 trees) and OOM-killed large runs. Results are byte-identical — split IDs are still assigned in tree-index order. ([#15](https://github.com/Joon-Klaps/rapidtrees/pull/15))
